...
from backend.quiz_bank_30 import CURATED_30_QUIZ_BANK

# Merge 30 comprehensive quizzes into the curated bank
CURATED_QUIZ_BANK.update(CURATED_30_QUIZ_BANK)

@router.get("/curated-bank")
@router.get("/categories")
async def get_curated_quiz_bank():
    """Lấy danh sách trọn bộ 30 chủ đề luyện tập trắc nghiệm đa dạng mọi cấp độ."""
    return {
        "count": len(CURATED_QUIZ_BANK),
        "categories": CURATED_QUIZ_BANK,
        "list": [
            {
                "id": k,
                "title": v.get("title", ""),
                ...
                "icon": v.get("icon", "🎯"),
                "level": v.get("level", "All"),
                "level_badge": v.get("level_badge", "badge-purple"),
                "category": v.get("category", "general"),
                "time_min": v.get("time_min", 15),
                "total_questions": len(v.get("questions", []))
            }
            for k, v in CURATED_QUIZ_BANK.items()
        ]
    }
...
async def get_quiz_category(cat_id: str):
    ...
    return {
        ...
        "icon": cat.get("icon", "🎯"),
        "level": cat.get("level", "All"),
        "level_badge": cat.get("level_badge", "badge-purple"),
        "category": cat.get("category", "general"),
        "time_min": cat.get("time_min", 15),
